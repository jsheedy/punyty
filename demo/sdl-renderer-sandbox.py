...
if __name__ == '__main__':
    ...
    while True:
        scene.update()
        scene.main_light = DirectionalLight(direction=Vector3(cos(0.2*time.time()), 0, sin(0.13*time.time())).normalize())
        t = time.time() - t0
        ...
        # )
        scene.main_camera.position = Vector3(20*math.sin(10*renderer.joystick.x), -5*10*renderer.joystick.y, 20*math.cos(10*renderer.joystick.x)) # p0.lerp(target, t/key) # + Vector3(0, 0, .1*cos(time.time()))
