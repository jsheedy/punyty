...
if __name__ == "__main__":
    ...
    cubes = (
        ...
        Cube(position=Vector3(2, 2, 0), color=Vector3(0.8, 0.0, 0.0)),
    )
    bunny = Model.load_ply("/Users/velotron/assets/stegs-low-poly.ply")
    bunny.color = Vector3(1, 0.5, 0.75)
    ...
    while True:
        scene.update()
        scene.main_light = DirectionalLight(
            direction=Vector3(
                cos(0.2 * time.time()), 1, sin(0.43 * time.time())
            ).normalize()
        )
        t = time.time() - t0
        bunny.color = Vector3(
            cos(0.2 * time.time()) / 2 + 0.5,
            cos(0.1 * time.time()) / 2 + 0.5,
            sin(0.4 * time.time()) / 2 + 0.5,
        )
        if t > key:
            t0 = time.time()
            t = 0
            ...
            target = Vector3(
                (d * random() + 1) * math.sin(pos),
                1 + 2 * random(),
                (d * random() + 1) * math.cos(pos),
            )
            key = 20 * random()
            p0 = scene.main_camera.position
        scene.main_camera.position = p0.lerp(target, t / key)
